...
fn compiler_import_fixture_verifies_targets_and_checked_expansion() {
    limits::deterministic(|| {
        ...
        verify_c0_prepared_sources_at(&proof, &imports, line, column)
            .expect("targeted imported proof");
        ...
        let reloaded =
            load_imports(&project.config()).expect("proof edits do not invalidate C input");
...
fn compiler_import_reproduces_header_existence_and_preserves_unsupported_bodies() {
    limits::deterministic(|| {
        ...
        assert!(
            verify_c0_prepared_sources(&project.proof().replace("15", "16"), &imports).is_err()
        );
    })
}
...
fn compiler_import_preserves_explicit_external_assumption_reporting() {
    limits::deterministic(|| {
        ...
        let imports = load_imports(&project.config()).unwrap();
        let assumptions =
            click::surface::c0_prepared_external_dependencies(proof, &imports).unwrap();
        assert_eq!(
...
fn userspace_compiler_import_refuses_profile_overrides_and_ambient_headers() {
    limits::deterministic(|| {
        ...
        for args in [
            ...
            assert!(
                error.contains("unsupported compiler argument")
                    || error.contains("cannot override"),
                "{args:?}: {error}"
            );
        }
        project.configure(1, &[]);
...
fn userspace_frozen_pthread_probe_records_real_header_boundary() {
    limits::deterministic(|| {
        ...
        configure_userspace(&project);
        create_lock(&project.config())
            .expect("prepare unchanged probe with real GCC/glibc headers");
        let imports = load_imports(&project.config()).unwrap();
        let lock: serde_json::Value = serde_json::from_slice(
            &fs::read(project.0.join("main.click.import.lock.json")).unwrap(),
        )
        .unwrap();
