...
- scalar declaration initializers such as `int32 i = 0;`, and one-dimensional
  scalar array initializers such as `int32 values[3] = {1, 2, 3};`
...
- `sizeof` for supported scalar and pointer types, and
  ...
  runtime-sized `int32` allocation such as `calloc(count, sizeof(int32))`, and
  standalone `free(pointer);`
...
them. The supported form is limited to `int32` elements and still requires a
positive, non-overflowing element count.
...
- array initializers for multidimensional arrays, structs, and other aggregates
