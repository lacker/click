...
- assignment-style `for (init; condition; step)` loops lowered to `while`,
  ...
  comma-separated sequence of scalar assignments or updates
- `return`
...
clauses and `continue` are not supported yet. The step may contain a
comma-separated sequence such as `i++, j--`; it is lowered in source order.
As in C, an `if`, `else`,
