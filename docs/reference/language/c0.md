...
`int32` cells fit in the new extent. The current slice does not preserve
zero-filled `calloc` prefixes or model arbitrary byte layouts.
...
conversions, allocator prototypes, casts, and zero-filled `calloc` prefixes
across `realloc` remain unsupported.
