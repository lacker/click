...
| `normalize()` | simple | Close a context-free proposition that normalizes to true. Top-level conjunction and disjunction construction is structural rather than one normalization leaf: use `both` for conjuncts, or first prove a disjunct and then use `left` or `right`. | Any goal that needs context fails. `normalize` does not construct a top-level conjunction or choose a top-level disjunct. Click runs the deterministic leaf check; expansion is unchanged, and profiling charges the normalized expression. Smart tactics retain these choices as `both` scopes or an explicit `have` followed by `left`/`right`. Other logical constructors remain migration compatibility while their smart-certificate paths move to explicit structure. | [`simple_tactics.md`](https://github.com/lacker/click/blob/master/mdtests/simple_tactics.md) |
| `normalize() using { P; ... }` | simple | Reduce a proposition using only explicitly cited conditions, then close by context-free normalization. | Every premise must be exactly available and express a single condition or its negation, including algebraic equality. Unlisted ambient facts are ignored. A top-level conjunction or disjunction remaining after reduction requires explicit structural construction. Reduction does not enter quantified bodies, match-arm bodies, or range-fold binders; introduce or unfold those first. Pure calls remain symbolic. Expansion retains the cited evidence, and work follows the selected expression and premises. | [`normalize_using.md`](https://github.com/lacker/click/blob/master/mdtests/normalize_using.md) |
