...
| `contradiction(P)` | simple | Close the goal from exact `P` plus exact `not P`, the opposite polarity of the same total C condition, or a context-free proof that `not P` is true. | A missing exact `P`, or a noncontradictory opposite, fails. Click performs indexed lookups and at most one context-free normalization; expansion is unchanged, and profiling charges one leaf. | [`surface_syntax.rs`](https://github.com/lacker/click/blob/master/src/surface/tests/surface_syntax.rs) |
