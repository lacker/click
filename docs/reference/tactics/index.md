...
| `match value { Type::Variant(fields) => { ... } ... }` | control | At unchanged function entry, split an ADT of any width into exhaustive execution proofs, one arm per constructor. The live arms are joined by a balanced tree of frontier splits, so three or more constructors need no extra wrapper functions. Fields are fresh typed, arm-local symbolic values, in scope for the whole arm including the arms of a `branch` or proof-level `if` written inside it and the body of a `have`; nested entry matches are supported. Each live arm must reach function exit and complete its proof. An impossible arm may consist of `contradiction(proposition)` naming an exact fact and its negation in that case; the remaining live arms are split and joined normally, and a sole live arm runs on the parent frontier without a join. | Missing, duplicate, wrong-type/arity, or escaping fields fail. Matching grants no memory: each live arm must independently unfold and restore its owned resources. Excluded arms retain checked contradictions, not simulated C outcomes. Expansion retains and checks every arm. Exclusion after local proof prefixes, all-impossible matches, post-state matches, and shared continuations remain unsupported. | [`proof_match_resource.md`](https://github.com/lacker/click/blob/master/mdtests/proof_match_resource.md), [`proof_match_three_constructors.md`](https://github.com/lacker/click/blob/master/mdtests/proof_match_three_constructors.md), [`proof_match_four_constructors.md`](https://github.com/lacker/click/blob/master/mdtests/proof_match_four_constructors.md), [`proof_match_live_arms_beside_an_excluded_one.md`](https://github.com/lacker/click/blob/master/mdtests/proof_match_live_arms_beside_an_excluded_one.md), [`match_bindings_in_branch_arm.md`](https://github.com/lacker/click/blob/master/mdtests/match_bindings_in_branch_arm.md) |
