...
| `normalize()` | simple | Close a context-free proposition that normalizes to true. A top-level conjunction is structural rather than one normalization leaf: prove it with `both`, or prove both conjuncts and use `split`. | Any goal that needs context fails. `normalize` does not construct a top-level conjunction proof. Click runs the deterministic leaf check; expansion is unchanged, and profiling charges the normalized expression. Other logical constructors remain migration compatibility while their smart-certificate paths move to explicit structure. | [`simple_tactics.md`](https://github.com/lacker/click/blob/master/mdtests/simple_tactics.md) |
| `normalize() using { P; ... }` | simple | Reduce a proposition using only explicitly cited conditions, then close by context-free normalization. | Every premise must be exactly available and express a single condition or its negation, including algebraic equality. Unlisted ambient facts are ignored. A top-level conjunction remaining after reduction requires `both` or `split`. Reduction does not enter quantified bodies, match-arm bodies, or range-fold binders; introduce or unfold those first. Pure calls remain symbolic. Expansion retains the cited evidence, and work follows the selected expression and premises. | [`normalize_using.md`](https://github.com/lacker/click/blob/master/mdtests/normalize_using.md) |
