...
| `step(callee(...), { binder: instance })` | simple | At an ordinary C call frontier, run one call to `callee` with every instance binder its sidecar declares bound to an instance the caller owns. `let name = step(callee(...), { ... })` introduces the instance a `produces` binder creates. | The frontier must call exactly that function with that many arguments. Missing, duplicate, unknown, wrongly typed, or unowned entries fail where they are written, as does an unintroduced `produces` binder. The map is the only source of bindings; nothing is searched or inferred. Unmapped caller instances frame. Expansion reprints the map and the `let` unchanged. | [`c_call_binder_transport.md`](https://github.com/lacker/click/blob/master/mdtests/c_call_binder_transport.md) |
