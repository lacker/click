...
| `step(Contract(...))` | simple | Explicitly apply a named contract with its declared proof arguments. | Empty applications are checked end to end. Resource arguments are parsed and checked for arity and resource family, but transport is not yet supported. No defaults or inferred arguments. | [`contract_explicit_empty_application.md`](https://github.com/lacker/click/blob/master/mdtests/contract_explicit_empty_application.md) |
...
| `arithmetic() using { P; ... }` | simple | Close the current signed `int32` goal using only the exact listed premises. Summed inequalities have positive unit coefficients; bounded nonlinear terms use interval facts; repeat a premise to spell a larger coefficient. Also closes a pointer-alignment goal `aligned(p + k, n)` from one listed `aligned(p, m)` base fact with `m >= n`. | An unavailable premise, unsupported relation, possible goal overflow, or failed consequence fails. Click scans only the explicit certificate, with work proportional to the listed premises and expression depth. | [`pure_arithmetic_affine.md`](https://github.com/lacker/click/blob/master/mdtests/pure_arithmetic_affine.md), [`pure_arithmetic_bounded_operations.md`](https://github.com/lacker/click/blob/master/mdtests/pure_arithmetic_bounded_operations.md) |
