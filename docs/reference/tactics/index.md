...
| `induct(xs) as ih { Type::Variant(fields) => { ... } ... }` | control | Prove a pure theorem for a recursive algebraic parameter by exhaustive constructor cases; each immediate recursive field admits `ih(field)`. | A missing, duplicate, unknown, or wrong-arity arm fails. Bindings are arm-local, and `ih` rejects nonrecursive or non-immediate values. Click checks every arm; expansion recurses through the arm certificates. | [`algebraic_structural_induction.md`](https://github.com/lacker/click/blob/master/mdtests/algebraic_structural_induction.md) |
| `apply(ih(m))` | smart | Select explicit proofs of `0 <= m`, `m < n`, and the theorem requirements at `m`, then instantiate the local induction hypothesis. | A bounded proof-selection miss, nondecreasing argument, or violated requirement fails. Click checks the selected operations; expansion adds any needed `have` steps and a `using` block. | [`pure_induction_countdown.md`](https://github.com/lacker/click/blob/master/mdtests/pure_induction_countdown.md) |
