...
| `match value { Type::Variant(fields) => { ... } ... }` | control | At unchanged function entry, split an ADT with one or two constructors into exhaustive execution proofs. Fields are fresh typed, arm-local symbolic values; nested entry matches are supported. Each live arm must reach function exit and complete its proof. An impossible arm may consist of `contradiction(proposition)` naming an exact fact and its negation in that case. | Missing, duplicate, wrong-type/arity, or escaping fields fail. Matching grants no memory: each live arm must independently unfold and restore its owned resources. Excluded arms retain checked contradictions, not simulated C outcomes. Expansion retains and checks every arm. Exclusion after local proof prefixes, all-impossible matches, wider joins, post-state matches, and shared continuations remain unsupported. | [`proof_match_resource.md`](https://github.com/lacker/click/blob/master/mdtests/proof_match_resource.md) |
