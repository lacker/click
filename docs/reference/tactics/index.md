...
| `unfold(function(args))` | simple | Expose one defining equation for the exact pure-function call at the current proof state and refresh the current proposition through that one layer. | An unknown function, argument mismatch, or unsupported non-`int32` recursive shape fails. Recursive calls in the body stay opaque; expansion is unchanged, and profiling charges the arguments and one body layer. | [`pure_function_unfold.md`](https://github.com/lacker/click/blob/master/mdtests/pure_function_unfold.md) |
