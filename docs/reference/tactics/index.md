...
| `arithmetic()` | simple | Close the current context-free signed-affine `int32` comparison or equality. Supported terms use addition, subtraction, and multiplication by constants. | A nonlinear or unsupported operator, a possibly overflowing goal expression, or a nonconsequence fails. Click checks only this goal and retains no arithmetic closure; expansion is unchanged. | [`pure_arithmetic_affine.md`](https://github.com/lacker/click/blob/master/mdtests/pure_arithmetic_affine.md) |
| `arithmetic() using { P; ... }` | simple | Close the current signed-affine `int32` goal using only the exact listed premises. Summed inequalities have positive unit coefficients; repeat a premise to spell a larger coefficient. | An unavailable premise, unsupported relation, possible goal overflow, or failed affine consequence fails. Click scans only the explicit certificate, with near-linear work up to map indexing factors. | [`pure_arithmetic_affine.md`](https://github.com/lacker/click/blob/master/mdtests/pure_arithmetic_affine.md) |
