...
| `left()` / `right()` | simple | Close the selected side of a disjunction from an exact available fact. | A missing selected fact fails even if the other side is available. Click checks the selected injection; expansion is unchanged, and profiling charges one leaf. | [`logical_tactics.rs`](https://github.com/lacker/click/blob/master/src/surface/tests/tactic_tests/logical_tactics.rs) |
...
| `cases (A or B) { ... } { ... }` | control | Eliminate an exact available disjunction, proving the goal once with `A` and once with `B`. | An unavailable disjunction or unfinished arm fails; this never substitutes `not A` for `B`. Click checks both arms, expansion recurses, and profiling reports descendants. | [`logical_tactics.rs`](https://github.com/lacker/click/blob/master/src/surface/tests/tactic_tests/logical_tactics.rs) |
