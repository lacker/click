...
| --- | --- | --- | --- | --- |
| `mark name;` | simple | At any live execution frontier, add a proof-local name for the current snapshot without advancing C. | A duplicate name fails. Replay records the exact snapshot; expansion leaves the simple step unchanged, and profiling attributes only its bounded checking work. | [`proof_mark_current_frontier.md`](https://github.com/lacker/click/blob/master/mdtests/proof_mark_current_frontier.md) |
...
| `transport(P, Q)` | smart | When `P` is available across certified snapshots, derive the equivalent target spelling `Q` by selecting frame evidence. | Unrelated propositions, unsafe writes, or a bounded evidence miss fail. Replay checks the chosen bridge; expansion adds `using`, and profiling reports search and leaves. | [`simple_statement_step_explicit_transport.md`](https://github.com/lacker/click/blob/master/mdtests/simple_statement_step_explicit_transport.md) |
| `transport(P, Q) using { R; ... }` | simple | Derive target proposition `Q` from available source `P` using exactly the listed snapshot and frame evidence. | An absent source, mismatched target, or insufficient evidence fails. Replay checks that bridge only; expansion is unchanged, and profiling charges the named evidence. | [`field_derived_precise_effect_after_metadata_write.md`](https://github.com/lacker/click/blob/master/mdtests/field_derived_precise_effect_after_metadata_write.md) |
