...
| `integer_certificate { premise ...; conclusion N; }` | simple | Check a flat, source-printable certificate over mathematical `Integer` comparisons. `premise` nodes name exact inputs, `scale`, `add`, and equality-bound nodes carry claimed results, and `trivial` handles context-free identities; `conclusion N;` selects the checked result. | Every result is recomputed, node references are forward-free and bounded by the explicit list, coefficients are exact constants, and every premise must be exactly available. The checker does not search ambient facts or use machine signed arithmetic. | [`integer_certificate_surface.md`](https://github.com/lacker/click/blob/master/mdtests/integer_certificate_surface.md) |
