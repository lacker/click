...
| `arithmetic() using { P; ... }` | smart | Plan and check a bounded signed `int32` certificate using only the exact listed premises. Summed inequalities have positive coefficients; bounded nonlinear terms use interval facts; one selected premise can be scaled to a larger coefficient. An equality goal is closed from two listed opposite non-strict bounds, and a listed strict comparison against a machine successor, `a < b + 1`, also reads as the non-strict bound `a <= b`, which needs no definedness premise because a wrapped successor makes the strict premise unsatisfiable. The successor shape is exact: `a < b + 2` is not decomposed. Also checks pointer alignment and equal byte translations of one listed pointer equality, including `p + 1 == arr + (i + 1)` from `p == arr + i` and `i < n`. Signed index sums are distributed only when the listed scalar bounds exclude overflow. | An unavailable premise, unsupported relation, possible goal overflow, or failed consequence fails locally. The planner scans only explicit premises and emits a flat certificate; expansion directly rechecks that certificate without planning again. | [`pure_arithmetic_affine.md`](https://github.com/lacker/click/blob/master/mdtests/pure_arithmetic_affine.md), [`pure_arithmetic_bounded_operations.md`](https://github.com/lacker/click/blob/master/mdtests/pure_arithmetic_bounded_operations.md), [`c_pointer_local_loop_invariant.md`](https://github.com/lacker/click/blob/master/mdtests/c_pointer_local_loop_invariant.md) |
...
| `close_invariants()` | smart | At a loop back edge, which is the body's end or a stepped `continue`, plan a checked body for the exact invariant value and safety obligations, and for the ranking members a `decreases` clause adds after them. A ranking member is closed by one `arithmetic() using` step citing the loop's declared invariants, its guard, and the function's written preconditions, and a tuple's pivot by `left`/`right`. | A bounded planning miss fails locally, and a ranked loop's diagnostic names the ranking members; a member needing an inequality outside that named set fails there rather than starting a search. Spell `close_invariants by { ... }` with explicit steps when needed. Expansion emits the retained body and never repeats invariant discovery while checking it. | [`c_decreases_count_up.md`](https://github.com/lacker/click/blob/master/mdtests/c_decreases_count_up.md) |
