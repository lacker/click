...
| `match value { Type::Variant(fields) => { ... } ... }` | control | At unchanged function entry, split an ADT with one or two constructors into exhaustive execution proofs. Fields are fresh typed, arm-local symbolic values; nested entry matches are supported. Each arm must reach function exit and complete its proof. | Missing, duplicate, wrong-type/arity, or escaping fields fail. Matching grants no memory: each arm must independently unfold and restore its owned resources. Expansion retains the match and checks its arm certificates. Contradictory-arm closure, wider joins, post-state matches, and shared continuations remain unsupported. | [`proof_match_resource.md`](https://github.com/lacker/click/blob/master/mdtests/proof_match_resource.md) |
