...
| `arithmetic()` | simple | Close the current context-free signed `int32` comparison or equality. In addition to affine terms, bounded products and constant remainder, shift, bit-mask, and sign-bit-flip terms are supported. The latter also checks unsigned constant bounds from explicit signed intervals. | An unsupported operator, a possibly overflowing goal expression, or a nonconsequence fails. Click checks only this goal and retains no arithmetic closure; expansion is unchanged. | [`pure_arithmetic_affine.md`](https://github.com/lacker/click/blob/master/mdtests/pure_arithmetic_affine.md), [`pure_arithmetic_bounded_operations.md`](https://github.com/lacker/click/blob/master/mdtests/pure_arithmetic_bounded_operations.md) |
| `arithmetic() using { P; ... }` | simple | Close the current signed `int32` goal using only the exact listed premises. Summed inequalities have positive unit coefficients; bounded nonlinear terms use interval facts; repeat a premise to spell a larger coefficient. Also checks pointer alignment and equal byte translations of one listed pointer equality, including `p + 1 == arr + (i + 1)` from `p == arr + i` and `i < n`. Signed index sums are distributed only when the listed scalar bounds exclude overflow. | An unavailable premise, unsupported relation, possible goal overflow, or failed consequence fails. Click scans only the explicit certificate; pointer translation collects exact offset terms without searching an equality graph. | [`pure_arithmetic_affine.md`](https://github.com/lacker/click/blob/master/mdtests/pure_arithmetic_affine.md), [`pure_arithmetic_bounded_operations.md`](https://github.com/lacker/click/blob/master/mdtests/pure_arithmetic_bounded_operations.md), [`c_pointer_local_loop_invariant.md`](https://github.com/lacker/click/blob/master/mdtests/c_pointer_local_loop_invariant.md) |
| `arithmetic_certificate { premise ...; conclusion N; }` | simple | Check the typed arithmetic-certificate envelope; its first rule family is mathematical `Integer` comparisons. `premise` nodes name exact inputs, `scale`, `add`, and equality-bound nodes carry claimed results, and `trivial` handles context-free identities; `conclusion N;` selects the checked result. | Every result is recomputed, node references are forward-free and bounded by the explicit list, coefficients are exact constants, and every premise must be exactly available. The checked family is retained in the proof object, and the checker does not search ambient facts or use machine signed arithmetic. | [`arithmetic_certificate_surface.md`](https://github.com/lacker/click/blob/master/mdtests/arithmetic_certificate_surface.md) |
