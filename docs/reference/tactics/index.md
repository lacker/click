...
| `arithmetic_certificate { premise ...; conclusion N; }` / `arithmetic_certificate signed_int32 { ... }` | simple | Check a typed arithmetic-certificate envelope. The unsuffixed form is the mathematical `Integer` family; `signed_int32` selects the checked signed-machine family, including affine, interval, definedness, bounded-operation, and proposition-conclusion nodes. | Every result is recomputed, node references are forward-free and bounded by the explicit list, coefficients and interval endpoints are exact, and every premise must be exactly available. The checked family is retained in the proof object; the checker does not search ambient facts or silently reinterpret a certificate across families. `integer_certificate { ... }` remains a parser-only compatibility spelling for the mathematical family. | [`arithmetic_certificate_surface.md`](https://github.com/lacker/click/blob/master/mdtests/arithmetic_certificate_surface.md), [`signed_int32_arithmetic_certificate_surface.md`](https://github.com/lacker/click/blob/master/mdtests/signed_int32_arithmetic_certificate_surface.md) |
