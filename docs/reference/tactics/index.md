...
| `close_invariants()` | smart | At a loop back edge, plan a checked body for the exact invariant value and safety obligations, and for the ranking members a `decreases` clause adds after them. | A bounded planning miss fails locally, and a ranked loop's diagnostic names the ranking members; spell `close_invariants by { ... }` with explicit steps when needed. Expansion emits the retained body and never repeats invariant discovery while checking it. | [`nested_loop_preserve.md`](https://github.com/lacker/click/blob/master/mdtests/nested_loop_preserve.md) |
