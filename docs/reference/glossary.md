...
inside the proof object. An **obligation** is a condition generated by semantic
...
syntax, usually a **proof script**. Checking evolves a persistent **proof
object**. Smart search extracts a **certificate** made of path-annotated
**certificate steps**. Replay checks those steps by constructing trusted
**kernel derivations**.
...
certificate step uses only the facts its surface form selects.
...
The ordered, path-annotated simple operations produced for replay. A smart
tactic is successful only if its certificate replays from the proof site's
initial state.

### Certificate step

One checked operation stored in a certificate. It records its selector and
structural path so replay applies it to the same fact, goal, resource, or
execution frontier for which it was produced. `SimpleProofStep` is the main
implementation type.
...
kernel rule and naming the exact premises it used. Certificate replay accepts
a step by constructing or checking these derivations, not by trusting the
planner that selected it.
...
The persistent, typed state that owns open goals and checked successor
transitions during proof construction. It is not the replay input: replay uses
the certificate extracted from checked operations. Kernel evidence types are
called [kernel derivations](#kernel-derivation) in prose.

### Proof script

An ordered Surface Click tactic block inside a proof clause. A script can mix
simple, smart, and control-flow tactics; it is source input, not a certificate.
...
smart site only after its certificate replays.
...
or record replayable evidence for it. Contrast with [definitional
...
Deterministic checking of a certificate from its initial state. Replay
...
site, or program point for an operation. Replay resolves a selector against
the current state and rejects it when the selected item is absent or
ambiguous.
...
A heuristic planner that searches for a sequence of simple tactics. Search is
untrusted; the resulting certificate steps must replay.
...
Producing a Surface Click form for a kernel proposition or term so a
certificate can cite it. It is a checked partial reverse of lowering: the
synthesized form must lower back to the same kernel meaning at the point where
the certificate uses it.
