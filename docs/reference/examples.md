...
- `examples/input-cursor/`: a struct holding a borrow. Each cursor owns its own
  metadata and views one shared input, its constructor keeps that input lent
  until the cursor is consumed, and `input_cursor_take` owns the cursor it
  advances.
...
- `examples/jsonc-existing-source/`: byte-preserved upstream json-c source
  with a checked SHA-256 manifest. Proves its numeric version and NUL-terminated
  version-string contents under the explicit unsigned-char kernel target.
