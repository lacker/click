...
resources to the fresh block. The current representation cannot express a
zeroed prefix followed by an uninitialized grown tail, so reallocating a
zeroed block is rejected until that state is modeled explicitly.
