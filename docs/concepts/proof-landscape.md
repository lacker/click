...
- A **control tactic** creates proof scopes or branches in which other tactics
  run. Structurally, this includes `have`, `open`, proof-level `if`, `cases`,
  `branch`, and `loop`. Timing and expansion additionally give the selectable `have` source
...
See the [proof tactics reference](../reference/tactics/index.md) for the exhaustive current
classification.
...
| Pointer viewability | C verifiers, separation logic | viewability and range axioms | `auto`, `execute()` | `mdtests/pointer_range.md`, `mdtests/pointer_range_missing_requires.md`, `mdtests/fill_n_symbolic_pointer_loop.md` |
...
| Existential proof | Lean `exists`/`cases`, Coq `exists`/`destruct`, Dafny witnesses | proof-local existential introduction and elimination | `witness(k = expr)`, `let (k: T) satisfy { P }` | `mdtests/witness_and_choose.md` |
...
| Frame reasoning | separation logic, C verifiers | pre/post memory evaluation with symbolic initial cells and loop write footprints | `auto`, contract `owns`/`views`, loop `owns` clauses | `mdtests/write_second_old_keeps_first.md`, `mdtests/write_second_old_rejects_overwritten_cell.md`, `mdtests/fill_n_loop_mutable_segment.md`, `mdtests/loop_frame_segment_shapes.md`, `mdtests/shifted_loop_effect_preserves_prefix.md` |
...
| Pure-function induction | Lean/Coq/Isabelle strong and structural induction | nonnegative `int32` induction plus exhaustive constructor induction over recursive algebraic values, with exact local-hypothesis instantiation | `induct(n) as ih`, `induct(xs) as ih { ... }`, `apply(ih(m))` | `mdtests/pure_induction_countdown.md`, `mdtests/pure_induction_two_step.md`, `mdtests/algebraic_structural_induction.md` |
