...
- C operators needed by ordinary library code:
  division/remainder, shifts, increments, compound assignment, ternary
  expressions, and broader `for` forms beyond the current assignment-only
  lowering.
- More integer types:
...
2. Extend the first C-string predicate layer toward a full string model. Open
   ...
   `valid_range` facts.
3. Decide and implement the first shift/integer-conversion slice. The open
   design question is how much of C's integer promotion and signed-shift
   behavior Click should model now versus reject in C0 until the integer story
   is broader.
