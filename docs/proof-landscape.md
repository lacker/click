...
| Assertions and facts | Lean/Dafny/F* proof scripts | spec assertion checking | `for statement(N) { assert ... by auto; }`, `have`, `assumption`, `derive` | `mdtests/count_to_three_loop_invariants.md`, `mdtests/count_to_three_bad_assert.md` |
...
| Rewriting and simplification | Lean `simp`, Isabelle simplifier | explicit equality substitution, atomic derivation, and simplification | `rewrite`, `normalize`, `derive`, `simp` | `mdtests/simple_tactics.md`, `mdtests/simp_postconditions.md`, `mdtests/pure_click_functions.md` |
