# Lifecycle ownership is not a memory view

```c filename=mutex_live_rejects_view.c
#include <pthread.h>
...
verifying "mutex_live_rejects_view.c";

void inner(struct holder *holder) {
    views mutex_live(&holder->mu);
} by { execute(); simp(); }
...
```expect
fail: mutex_live authority is owned and cannot be viewed or duplicated
```
