# Attaching without retain cannot create a child reference

The C attach stores the pointer but omits the retain. Its contract still
claims to produce a second `child_ref(kid)`; certification must refuse that
claim before the caller can detach.

```c filename=shared_heap_missing_retain.c
...
verifying "shared_heap_missing_retain.c";
...
```expect
fail: could not certify contract for `parent_attach`
```
