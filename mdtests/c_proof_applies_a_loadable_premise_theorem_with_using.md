# A C proof applies a `loadable`-premise theorem with `using`

`mdtests/sweep_maintains_a_zero_unmarked_count.md` applies a theorem with
`loadable` premises from a C proof, and states every extent bound the
...
```text
theorem `...` requirement 1 `(n >= 0 && loadable(v[0..n]))` states a memory
...
Citing a stated range cites what the range says. `loadable(p[a..b])` means both
that `a..b` is a valid 32-bit byte extent and that those bytes are loadable, so
...
theorem loadable_range_is_nonnegative(v: int32[], n: int32) {
...
int32 probe(int32 a[], int32 n) {
    ...
    step();
    have n >= 0 by { simp(); }
    have loadable(a[0..n]) by { simp(); }
    have n >= 0 and loadable(a[0..n]) by { split(); }
    apply(loadable_range_is_nonnegative(a, n)) using {
        n >= 0 and loadable(a[0..n]);
    }
    simp();
}
```
