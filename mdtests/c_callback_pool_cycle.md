# Acquire, mutate, and release through two abstract callbacks

This one-slot pool uses an opaque availability token. Acquisition either returns
that token on failure, or transfers a cell and a pool-specific lease on success.
Release consumes the cell and lease and restores availability. The storage is
not assumed fresh: the caller explicitly permits writes to the pool's slot.
