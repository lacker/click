...
whole-loop `owns` clause plus the separated source/destination requirement lets
...
int32 copy_n_segment_invariant(int32 dst[], int32 src[], int32 n) {
    ...
    have forall (k: int32) { 0 <= k and k < n implies src[k] == old(src[k]) } by {
        have forall (k: int32) {
                at(statement(0).entry, 0) <= at(statement(0).entry, k) and
                at(statement(0).entry, k) < at(statement(0).entry, n) implies
        ...
        transport(
            ...
            forall (k: int32) {
                0 <= k and k < n implies old(src[k]) == src[k]
            }
        ) using {
            forall (k: int32) {
                    at(statement(0).entry, 0) <= at(statement(0).entry, k) and
