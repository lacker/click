...
the proposition `loadable(v[lo..hi])` states, and carries the same valid-extent
...
a value only where that cell is loadable, and the cell comes from the stated
...
The second theorem writes the same premise as a bare `requires loadable(...)`.
...
premise is named there by its fact form, `loadable(v[0..hi])`, and the two
