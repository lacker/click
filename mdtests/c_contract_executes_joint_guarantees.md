...
contract void First(int32* p) { owns p[0..2]; ensures p[0] == 1; }
contract void Second(int32* p) { owns p[0..2]; ensures p[1] == 2; }
contract void Buffered(int32* p) {
    owns Buffer(p); ensures p[0] == 1; ensures p[1] == 2;
}
theorem lift(callback: void (*)(int32*)) executes callback(int32* data) {
    ...
    ensures Buffered(callback) by {
        unfold(Buffer(data)); step(First); fold(Buffer(data)); simp();
    }
}
verifying "joint.c";
void invoke(void (*callback)(int32*), int32* data) {
    requires First(callback);
    requires Second(callback);
    owns Buffer(data); ensures data[1] == 2;
