...
the bounded length, the loadability of each preceding byte, and the
loadability of the terminator. The first three `have`s state those exact
...
int32 fixed_string_length(uint8 bytes[]) {
    ...
    ensures result >= 0 by {
        ...
        have exists (len: int32) {
            defined(len + 1) and loadable((bytes + len)[0..1])
        } by {
            witness(len = 0);
            both {
                simp();
                ...
                transport(
                    at(function.entry, loadable(bytes[0..3])),
                    loadable((bytes + 0)[0..1])
                ) using {
                    at(function.entry, loadable(bytes[0..3]));
                }
            }
