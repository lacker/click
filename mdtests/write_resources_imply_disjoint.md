# write resources imply disjoint

This checks that two held `write(...)` resources imply a `disjoint(...)` fact
for their ranges. No explicit `disjoint(...)` requirement is needed here.
