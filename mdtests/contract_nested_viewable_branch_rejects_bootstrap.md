# A conditional viewability atom is not entry read authority

Viewability under an `or` branch is conditional.  It must not bootstrap the
...
contract void NestedLoadableBranch(struct node* node, int32 n) {
    requires node != 0;
    requires n >= 1 and (n == n or viewable(node[0..n]));
    owns cell(node->left);
}
```
