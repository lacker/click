...
```expect
fail: could not evaluate the contract entry resources: function contract could not be applied: could not evaluate `node->left->left` while checking `owns pair(node->left->left)`
```
