...
helper is allowed to write `p[0]`, but it consumes that permission instead of
returning it, so the caller cannot prove it still has the permission after the
call.
