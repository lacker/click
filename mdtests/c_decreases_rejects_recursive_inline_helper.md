...
```expect
fail: could not certify termination for `drain`: callee `step_down` has no termination evidence: the recursive call to `step_down` is ranked by no function-level `decreases` measure
```
