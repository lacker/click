# `rb_first` may not claim a first element of the empty tree

The negative for [`rb_first_last.md`](rb_first_last.md)'s in-order clause. The
guard `old(t.model) != RbTree::Empty` is not decoration: on an empty tree
`rb_first` returns null, `rb_inorder(RbTree::Empty)` is the empty list, and no
node is its first element. Dropping the guard claims that the returned pointer
is first in every case, and the empty path cannot establish it.
...
```c filename=rb_first_not_first.c
#include "rbtree.h"
...
```click
verifying "rb_first_not_first.c";
...
struct rb_node* rb_first(const struct rb_root* root) {
    ...
    ensures rb_list_starts_with(rb_inorder(old(t.model)), result) == 1;
} by {
    step();
    step();
    match t.model {
...
```expect
fail: `ensures rb_list_starts_with(rb_inorder(old(t.model)), result) == 1` failed for `rb_first.ensures_4`
```
