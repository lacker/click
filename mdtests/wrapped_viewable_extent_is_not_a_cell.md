# a wrapped viewable extent must not yield a real cell

A `viewable` range becomes a byte extent `(end - start) * width`, in modular
32-bit arithmetic. At `n == 1 << 30` with four-byte elements that product is
`1 << 32`, which is `0`, so `viewable(v[0..n])` claims an empty extent and is
...
zero bytes, and claiming those bytes are viewable claims nothing. It stays here
