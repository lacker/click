# A loop cannot silently replace an existing mutex initialization

```c filename=modeled_pthread_reinitialized_loop.c
#include <pthread.h>
...
```expect
fail: Click does not yet support destroying and reinitializing a loop-head mutex across a loop backedge; the loop requires the same initialization
```
