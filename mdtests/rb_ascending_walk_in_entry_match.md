# a ranked loop inside a proof match arm

This is [`rb_ascending_walk_to_root.md`](rb_ascending_walk_to_root.md) with its
whole proof moved inside one entry `match t.model`, which is the shape
`__rb_insert` has: the insert fixup decides its cursor's constructor once at
entry and runs its ranked `while` loop inside that arm.

The arm's constructor equation is a *path premise*, not a different entry
state. A checked execution that begins by assuming `t.model ==
RbTree::Node(..)` still starts at the contract's entry state; contract
certification rebases it by discharging the arm's case premise against the
contract candidate rather than by demanding that the two entry states be
identical. Before package A26 this refused with `the checked execution of
'rb_root_of' started at a different entry state than the contract and could not
be rebased onto it`.

The model, the resources and the loop proof below are unchanged from the
unwrapped fixture, which stays as the flat regression. The parent is a
*payload*, not a resource parameter: `rb_at`'s `Node` arm owns
...
```c filename=rb_ascending_walk_in_entry_match.c
#include "rbtree.h"
...
```click
verifying "rb_ascending_walk_in_entry_match.c";
