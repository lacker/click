# Lifecycle authority cannot be counted

```c filename=mutex_live_rejects_quantity.c
#include <pthread.h>
...
resource lifetime(holder: struct holder*) {
    field tag: int32;
    owns 2 of mutex_live(&holder->mu);
}

verifying "mutex_live_rejects_quantity.c";
...
```expect
fail: `mutex_live` is exclusive and not countable
```
