...
current Linux parser boundary is the GNU `__const__` function attribute in
`/usr/include/time.h:80`; bare `struct sigevent;` and the inline pointer
array in `bits/types/__locale_t.h:30` now parse. The
