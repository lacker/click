# Preserve structured loop exits and model `for` `continue`

Found by the 2026-09-01 kernel audit at cb034b21.

The initial native control-flow slice now models `break`, `continue`, a
bounded `switch`, and post-tested `do ... while` loops in the kernel and
surface proof path. `continue` in a `for` loop is still rejected because its
...
or a `switch` over an integer verifies without rewriting. A `for` loop's
`continue` must execute its update clause before retesting its condition.
...
4. `do { body; } while (condition);` with a `continue` edge to the condition
   and a `break` edge to the following statement (`mdtests/c_do_while.md`).
