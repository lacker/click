...
**Status: recovery handoff required. V0-V17 have green integrated checkpoints,
but the current V18 worktree is a large, uncommitted experiment and is not a
green checkpoint. It implements the approved top-level borrowed-input root and
reduces the candidate corpus substantially, but it also mixes several later
compatibility fixes and two incomplete soundness investigations. Do not begin
V19 or integrate this worktree until the recovery audit below has reduced it
to reviewable commits and the candidate corpus is green. Ordinary C contracts
still use the existing weak-view behavior until the V19 cutover.**
...
### 2026-09-13 recovery handoff for the uncommitted V18 worktree

The active worktree is `/private/tmp/click-fix-views-v7` on branch
`codex/fix-views-v7`. Its last integrated base is `b2419452`. The primary
checkout must remain integration-only. At this handoff, 20 files are modified
and there is no commit containing the current changes. `cargo check` passed
before the latest return-provenance experiment; `scripts/check.sh` has not been
run on this worktree. Treat every uncommitted edit as suspect until the focused
tests and diff audit below establish otherwise.

The worktree currently contains these coherent ideas:
...
- A `BorrowedContractInput` loan origin models a top-level `views`
  ...
  ownership. This is internal kernel state; it adds no language syntax.
- Contract-proof entry installs those roots after argument/resource lowering.
  Composite input views record a definition-checked one-level child frontier,
  and bindings follow exact child occurrences rather than fact equality.
- Direct, named, callback, certification, proof, and loop paths preserve the
  ledger, participant, and occurrence-binding sidecar instead of replacing the
  resource context directly.
- Stores, free, realloc, call effects, and loop havoc consult active stable
  loans. Symbolic borrowed ranges are retained outside the concrete dyadic
  index and checked with current separation assumptions.
- Candidate verification is wired through mdtests and examples. Inline
  helpers use their caller's checked resources. Return/certification rebases
  preserve bound occurrence identities.

These changes have focused evidence: the primitive external-root tests pass;
ordinary and nested symbolic readers pass; partial views, view-supported
facts, recursive resource proof operations, `requires_memory_read`, and the
two loop range examples `copy_n_segment_invariant` and
`shifted_copy_effect_uses_covering_separate` passed individually. The exact
principal contract-input root succeeds, while derived or ambiguous root
selection is rejected.

The last complete candidate corpus run is
`/tmp/click-md-candidate-5.log`: 33 of 1,485 mdtests failed, down from 161 in
the first run and 53 in the preceding checkpoint. That log predates the latest
intrinsic-read and interval-relation edits, so it is a baseline rather than a
verdict. The current changes subsequently made bounded local views use the
existing activation-local bounds rule and made exact const/static memory views
intrinsic read-only authority. Focused tests for
`borrowed_local_view_in_bounds`, `borrowed_local_view_bounds_rejected`,
`local_array_decays_to_helper`, `const_global_table`,
`const_callback_table_abstract`, and `rb_augment_callbacks_table` pass.

The unresolved corpus is grouped as follows:

1. **Returned-view provenance and selected mutable footprints.** The range
   relation initially failed to recognize adjacent ranges with a common
   symbolic base. A local structural comparison now reaches return recovery,
   where `c_step_contract_selected_footprint` exposes two output-view sources:
   the preserved outer borrowed view and a view deduplicated against a checked
   returned owner. `parent_binding_for_view` was added for the first case. The
   second case still needs an explicit checked provenance record; do not accept
   it merely because some owner happens to satisfy the returned view. The
   temporary diagnostic used to identify both sources has been removed.
   Related fixtures include
   `c_named_function_contract_pipeline`,
   `modular_call_requirement_indexes_footprint`, and the callback footprint
   compatibility cases.
2. **Recursive composite views.** `augment_rotate_callback*` needs a view of
   recursive `shape`. The existing finite composite loan intentionally rejects
   nested or undecidable frontiers. A proposed patch that simply admitted
   composite child pieces was rejected by automatic approval review because it
   did not protect the complete recursive footprint. The next design must use
   a distinct opaque recursive capability with an enforcing write/lifetime
   rule, or retain the explicit unsupported boundary. Do not flatten a
   recursive resource or remove the rejection.
3. **Composite body facts.** `composite_resource_view_then_mutate`,
   ...
   `opaque_calls_preserve_public_store_fact` still hit the deliberate rejection
   for fact-bearing composite backing. Supporting them requires recording the
   exact checked facts and the stable memory/resource dependencies that justify
   reusing them. Removing the guard alone is unsound.
4. **Loops and branches.** Loop havoc must use the checked owned entry
   footprint as `validated_ranges`; unknown write sets must continue to fail
   closed. Validate a loop's declared ownership before stable-loan havoc so the
   ownership diagnostic is not masked. `proof_branch_pointer_local` retains a
   binding sidecar while one abstraction path directly empties resources, then
   compares it with `with_resource_context`, which rebuilds the sidecar. The
   join needs an authority-aware comparison, with positive same-root and
   negative changed/dropped-root regressions.
5. **Certification regressions.** `struct_conditional_value` now passes entry
   root selection but reports that no checked execution matches its execution
   mode. `pure_click_functions` fails only the stdlib `count` claim. Audit the
   new `with_resource_context` rebases and the distinction between local
   aggregate views, stdlib lowering, and real external roots.
6. **Expected negative diagnostics.** Several negative fixtures now fail at
   the stable-loan barrier before reaching their historical missing-ownership
   ...
   the true violated invariant and the corresponding positive behavior is
   already sound.

The symbolic-range code needs cleanup before more semantics are added.
`LoanLedgerData` now has `unindexed_memory`, an `indexed_memory` fallback map,
and counters. The second map was introduced to conservatively check a symbolic
write query against concrete active roots, but it has not passed a focused
test or scaling review. Its invariant code is awkward and should be rewritten
or reverted. Preserve the fail-closed behavior of `active_memory_overlaps`;
only the assumption-aware access path may use a bounded explicit fallback.
The repository efficiency contract forbids an ambient whole-ledger scan on a
common path, so any fallback must be charged to the explicit symbolic query or
indexed by block.

Two automatic approval reviews are material constraints on this handoff:

- A prototype that rooted every view already present in the state was rejected
  because authority may be created only for exact checked contract-input
  occurrences. The current principal-occurrence selection is the safe repair;
  do not restore ambient enumeration.
- Broadening ordinary composite backing to nested composites was rejected
  because the full recursive footprint was not protected. Introduce a distinct
  checked representation and enforcement rule before supporting this case.

Recovery should proceed in this order:

1. Remove temporary diagnostics, inspect every current diff, and make
   `cargo check` plus the loan unit tests green. Revert the incomplete
   `indexed_memory` and return-provenance experiments if their invariants
   cannot be stated and tested locally.
2. Split the work into reviewable commits: external contract roots and exact
   bindings; symbolic range/access enforcement; local and read-only intrinsic
   views; then proof/certification rebases. Run focused positive and negative
   tests for each commit.
3. Rerun the complete candidate mdtest and example corpora. Reclassify the
   remaining failures from the fresh log rather than relying on the list above.
4. Finish returned-view provenance, loop/branch authority, and certification
   regressions before attempting recursive or fact-bearing composites.
5. Design recursive and fact-bearing composite capabilities explicitly and
   subject them to an adversarial review. They are required for the rbtree
   launch path, so an unsupported result is not V18 completion.
6. Only after the candidate corpus is green, assign a fresh V18 adversarial
   review. V19 then removes the legacy selector, makes stable views the only
   ordinary-memory meaning, updates durable docs, runs `scripts/check.sh`, and
   deletes this issue and its README entry.

The immediate recovery agent should optimize for a small, understandable,
green checkpoint. It should not drive the failure count down by weakening a
guard, changing C, accepting a view by fact equality, or updating many expected
messages at once.
