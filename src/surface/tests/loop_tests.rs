...
fn smart_ranking_closure_expands_to_explicit_bundle_members() {
    ...
    // The two ranking members, and the invariant `i <= n`, which the
    // certificate closes since `i < n` and `n <= 2147483647` together bound
    // `i + 1`; it needed a lemma application while the planner bounded each
    // atom only by premises naming it alone.
    assert_eq!(
        arithmetic_certificate_premises(closer).len(),
        3,
        "both ranking members and the upper-bound invariant must be closed by one signed certificate each: {expanded}"
    );
    let (result, planning) = crate::surface::proof::count_planning_statement_transitions(|| {
        verify_c0_sources(&expanded, &sources)
    });
