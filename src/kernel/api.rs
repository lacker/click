...
pub(crate) fn c_state_with_borrowed_contract_inputs(
    ...
    if !function.resource_requires().iter().any(CResourceSpec::is_view) {
        return Ok(state);
    }
        ...
        || !state.loan_bindings_are_consistent()
    {
        return Err(LoanRefusal::InvalidEvidence
            .diagnostic(LoanRefusalOperation::Entry));
    }
    let entry = c_function_entry_state(&state, function, arguments).ok_or_else(|| {
    ...
    for input in checked_inputs.into_iter().filter(|input| input.fact.is_view()) {
        if matches!(
            input.fact.resource(),
        ...
        let candidates = state
            ...
            .filter(|occurrence| {
                state
                    .resources()
                    .view_occurrence_is_principal(*occurrence)
            })
            .collect::<Vec<_>>();
    ...
    for (occurrence, viewed) in selected {
        ...
        for piece in backing_pieces {
            let occurrences = state.resources().view_occurrences_for_fact(&piece, assumptions);
