...
thread_local! {
    ...
    static CONTEXTUAL_SIMPLIFIER_TERM_VISITS: Cell<usize> = const { Cell::new(0) };
    static DIRECT_CONSTANT_EQUALITY_FACT_VISITS: Cell<usize> = const { Cell::new(0) };
    static CONTEXTUAL_EQUALITY_VERTEX_VISITS: Cell<usize> = const { Cell::new(0) };
    static CONTEXTUAL_LOWERING_ROUNDS_EXECUTED: Cell<usize> = const { Cell::new(0) };
}
...
pub(crate) struct ContextualLoweringWork {
    pub(crate) simplifier_term_visits: usize,
    pub(crate) direct_equality_fact_visits: usize,
    pub(crate) equality_vertex_visits: usize,
...
impl PureFactContext {
    ...
    #[cfg(test)]
    pub(crate) fn reset_contextual_lowering_work() {
        CONTEXTUAL_SIMPLIFIER_TERM_VISITS.with(|visits| visits.set(0));
        DIRECT_CONSTANT_EQUALITY_FACT_VISITS.with(|visits| visits.set(0));
        CONTEXTUAL_EQUALITY_VERTEX_VISITS.with(|visits| visits.set(0));
        CONTEXTUAL_LOWERING_ROUNDS_EXECUTED.with(|rounds| rounds.set(0));
    }

    #[cfg(test)]
    pub(crate) fn contextual_lowering_work() -> ContextualLoweringWork {
        ContextualLoweringWork {
            simplifier_term_visits: CONTEXTUAL_SIMPLIFIER_TERM_VISITS.with(Cell::get),
            direct_equality_fact_visits: DIRECT_CONSTANT_EQUALITY_FACT_VISITS.with(Cell::get),
            equality_vertex_visits: CONTEXTUAL_EQUALITY_VERTEX_VISITS.with(Cell::get),
            rounds: CONTEXTUAL_LOWERING_ROUNDS_EXECUTED.with(Cell::get),
        }
    }
    ...
    /// Chooses a preferred lowering of `term` from the closure generated by
    /// recorded-equality edges and contextual simplification: constants
    /// first, then the member mentioning the fewest memory loads, ties broken
    /// by term order. Unlike
    ...
    pub(in crate::kernel) fn lower_bitvector_via_recorded_equalities(
        ...
        while let Some(current) = stack.pop() {
            ...
            #[cfg(test)]
            CONTEXTUAL_EQUALITY_VERTEX_VISITS.with(|visits| visits.set(visits.get() + 1));
            ...
            // the member, so it joins the walk. The visited set prevents an
            // exact term from recurring, but generated simplified terms can
            // bridge equality components, so the recorded class size does not
            // bound this closure.
    ...
    /// simplification. Applied to a verified call's mutable footprint after
    /// its resource segment has been evaluated, so downstream frame queries
    /// can compare the selected contextual vocabulary syntactically.
    ...
    /// changing `self` can change the result. The equality worklist itself
    /// keeps adding simplified members, so the three outer rounds do not
    /// bound equality/simplification alternation or its search work.
    pub(in crate::kernel) fn lower_bitvector_under_assumptions(
        &self,
        term: &Bitvector32Term,
    ) -> Bitvector32Term {
        // Structural simplification rewrites inside a class member
        // (`new_len` -> `old_len + 1` -> `1`), while class selection swaps
        // whole terms. The inner equality worklist already closes over newly
        // simplified members; these rounds only reapply the preference-gated
        // result and are not a meaningful search bound.
        const CONTEXTUAL_LOWERING_ROUNDS: usize = 3;
        let mut term = term.clone();
        for _ in 0..CONTEXTUAL_LOWERING_ROUNDS {
            #[cfg(test)]
            CONTEXTUAL_LOWERING_ROUNDS_EXECUTED.with(|rounds| rounds.set(rounds.get() + 1));
