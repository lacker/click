...
/// Checks one source-linear loop-preservation script on a persistent Proof.
/// The loop body frontier, explicit operations, smart descendants, invariant
/// judgment, and retained provenance all remain on that lineage. A structural
/// source shape or a transactional smart miss returns `None` from the
/// unchanged root so the still-transitional frontier-loop driver can own it.
#[allow(clippy::too_many_arguments)]
pub(in crate::lang::click::proof) fn try_check_linear_loop_preservation<'a>(
    root: Proof<'a>,
    ...
    let at_back_edge = {
        ...
        matches!(
            &view.replay.frontier.point,
            ProofExecutionPoint::StatementEntry { remaining }
                if remaining.as_ref() == sentinel
        ) && view.replay.frontier.continuations.is_empty()
    };
    if !at_back_edge {
        return Ok(None);
    }

    let selected_simp = match remaining.as_slice() {
        [] => None,
        [indexed] if matches!(indexed.tactic, ProofTactic::Simp) => Some(indexed),
        _ => return Ok(None),
    };
    let checkpoint = proof.checkpoint();
    if let Some(indexed) = selected_simp {
        proof = proof.with_execution_tactic_index(indexed.index)?;
    }
    proof = proof
        .certify_loop_invariant_bundle(loop_entry_state, invariant_checks)
        .map_err(|error| {
            ClickError::new(format!(
                "`{claim_label}` invariant bundle preservation: {}",
                error.message()
            ))
        })?;
    if let Some(indexed) = selected_simp
        && indexed.source_index != owning_source_index
        && let Some(site) = proof_site
    ...
    if let (Some(expansion_capture), Some(staged)) =
        (expansion_capture.as_deref_mut(), staged_expansion_capture)
    {
