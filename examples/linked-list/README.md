...
Nodes are supplied by the caller. `list_empty` receives a typed pointer proved
equal to null, avoiding any dependency on C integer-to-pointer conversion.
`list_push_front` transfers a detached node and an existing list into a new
list. `list_pop_front` performs the reverse ownership transfer while leaving
the old `next` value in the detached node. Explicit `fold` and `unfold`
