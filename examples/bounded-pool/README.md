...
resource. Transfer consumes `pool_object(source, object)` and produces
`pool_object(destination, object)` while decrementing the source count and
incrementing the destination count. Return consumes the resource and gives the
object memory back.
